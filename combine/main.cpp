...
int main(int argc, char *argv[]) {
	...
	if (FLAGS_lock == "Mutex") {
	  ...
	  RWLock *lock = new(buffer) RWLock();
		...
		experiment(lock);
		delete lock;
	} else if (FLAGS_lock == "Test") {
		testgroup_t *group = testgroup_default();
		testgroup_set_exp_freq(group, FLAGS_expFreq);
		testgroup_set_exp_time(group, FLAGS_expTime);
		TestLock *lock = new(buffer) TestLock(group);
		experiment(lock);
		delete lock;
